...
def get_random_walk():
    ...
    with open("./dataset/data_10_random_walk_100.txt","w") as fout:
        walk_L=100
        walk_n=10
                ...
                if node.ntype[0]=='d' and len(node.getTotalAdj()) >0:
                    ...
                    while len(walk_list)<walk_L:
                        ...
                        if rand_p > 0.5:
                            ...
                            while len(tmp_node.getTotalAdj()) ==0:
...
def get_prob():
    with open("./dataset/data_5_random_walk_100.txt", "r") as fin:
...
def dataset_build():
    ...
    with open("./dataset/data_5_walk_100.txt", "w") as fout:
    ...
    for i in range(len(records)):
        if records[i].RspStatus=='2':
            ...
            if records[i].queryType=='1'  and records[i].answer_rrs!='0':
                #处理资源记录[个数，（type,name,ttl）,...]
                #DNSClientIPLower和records[i].Domain添加query联系
                    ...
                    for record in result:
                        ...
                        if rtype=='CNAME':
                            ...
                            if tmpnode in node2indexdict:
                                ...
                                index2nodedict[index]=tmpnode
                                #records[i].Domain和value之间添加cname联系
                            ...
                            #records[i].Domain和value之间添加map联系
                a_count+=1
                domainset.add(records[i].Domain)
            ...
            if records[i].queryType=='1':
                ...
                index2nodedict[from_client_node].addAdj('domain',query_domain_node)
                index2nodedict[query_domain_node].addAdj('client',from_client_node)    
                #print(records[i].Rsperrorcode)# 为什么请求失败的查询也有返回别名资源记录
...
benign_domain_list=[]
for node in node2indexdict:
    if node.ntype=='domain' and node.value in ben_set:
        benign_domain_list.append(node)
random.shuffle(benign_domain_list)
prune_domain_set=prune_domain_set.union(set(benign_domain_list[433*10:]))
